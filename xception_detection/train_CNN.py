...
def main():
	...
	for _, targets in train_loader:
		for target in targets:
			zero_counter += int((1-target))
			one_counter += int((target))
	...
	model = model.cuda()
	#weight = torch.FloatTensor([one_counter/train_dataset_size, 0.5]).cuda()
	criterion = nn.CrossEntropyLoss()#weight=class_weights) #weight=torch.tensor([1, weight_numb]).cuda())
...
if __name__ == '__main__':
	...
	parse.add_argument('--epoches', '-e', type=int, default='12')
